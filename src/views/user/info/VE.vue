              ...
              <div style="width: 100px;flex-shrink: 0;">{{ index }}</div>:
                <div>
                  <div style="margin:0 20px 20px 20px;" v-for="(data2,index2) in data" :key="index2">
                    {{ index2+1 }}. | 所在地区：{{ data2.info }} | 详细地址：{{ data2.detail }} | 收件人：{{ data2.name }} | 电话：{{ data2.phone }} <div v-if="data2.is_default" style="margin: 5px 0 0 30px;display: inline-block;background-color: rgba(127, 255, 212, 0.648);border: 1px rgba(0, 0, 0, 0.171) solid;border-radius: 5px;padding: 5px;color: rgb(14, 198, 249);">默认</div>
                  </div>
                </div>
              </div>
          ...
          <div style="margin:0px 20px ;">
            ...
            <div v-else-if="index==='addresses'">
              <div style="margin:10px 20px 20px 10px;" v-for="(data2,index2) in data" :key="index2">
                {{ index2+1 }}. | 所在地区：{{ data2.info }} | 详细地址：{{ data2.detail }} | 收件人：{{ data2.name }} | 电话：{{ data2.phone }} <div v-if="data2.is_default" style="margin: 5px 0 0 30px;display: inline-block;background-color: rgba(127, 255, 212, 0.648);border: 1px rgba(0, 0, 0, 0.171) solid;border-radius: 5px;padding: 5px;color: rgb(14, 198, 249);">默认</div><el-button @click="change_address(index2,data2)" style="margin-left: 20px;" size="mini" type="primary" icon="el-icon-edit" circle></el-button><el-button size="mini" @click="delete_address(index2)" type="danger" icon="el-icon-delete" circle></el-button>
              </div>
    ...
    <el-dialog :title="this.dialog_title+this.dialogindex" :visible.sync="dialogVisible" width="40%" >
      <el-form ref="form" :model="dialogdata" label-width="80px" >
        <el-form-item label="所在地区">
          <el-input v-model="dialogdata.info"></el-input>
        </el-form-item>
