<template>
  <div :style="'display: flex;padding: ' + padding + ' 0 5px 0;'">
      ...
      <div style="margin-left: 50px;">
        ...
        <div v-else class="mobile" style="font-size:13px;color: #9499A0;padding-right: 16px;margin: 0;">
          ...
          <div style="display: inline-block;margin: 0;padding: 0;width: 150px;">
            <div v-if="scope.sub_num && !amisub" v-b-toggle="'collapse-' + scope.id" @click="getsubbyid(scope.id)" style="display: inline;cursor: pointer;">
      ...
      <div style="margin-left: 20px;">
        ...
        <b-collapse v-if="!amisub && scope.sub_num && true" :id="'collapse-' + scope.id" class="m-1 collapse-loading" >
          <b-card v-loading="this.subloading" style="height: 100%;border: none;">
            ...
            <!-- 上拉收起 -->
            <div v-b-toggle="'collapse-' + scope.id" @click="getsubbyid(scope.id)" style="display: flex;justify-content: end;font-size:13px;color: #9499A0;margin: 0px 30px 3px 0;">
                <div class="hoverable">上拉收起评论<i style="padding-left: 2px;" class="el-icon-arrow-up"></i></div>
            </div>
    ...
    <!-- 点评论图标 弹出输入 -->
    <van-popup v-model="van_popup_show" position="bottom" style="height: 200px;background-color: rgb(243, 243, 244);" >
